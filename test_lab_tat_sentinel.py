import pytest
from lab_tat_sentinel import QueueBottleneckForecasterAgent, SLABreachPredictorAgent, AnalyzerWorkloadBalancerAgent, LISOperationsCoordinator, main


def test_sub_agents():
    a1 = QueueBottleneckForecasterAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = SLABreachPredictorAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = AnalyzerWorkloadBalancerAgent()
    alerts3 = a3.evaluate({"status_text": "DISCORDANT_FINDING"})
    assert len(alerts3) == 1


def test_coordinator():
    coord = LISOperationsCoordinator()
...
def test_domain_registry():
    from lab_tat_sentinel import DomainKnowledgeRegistry
